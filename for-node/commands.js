...
async function add({ gitdir, workdir, filepath }) {
  const type = 'blob';
  const object = await utils_js.read(path.join(workdir, filepath));
  if (object === null) throw new Error(`Could not read file '${filepath}'`);
  const oid = await managers_js.GitObjectManager.write({ gitdir, type, object });
  await managers_js.GitIndexManager.acquire(`${gitdir}/index`, async function (index) {
    let stats = await pify(utils_js.fs().lstat)(path.join(workdir, filepath));
...
async function checkout({ workdir, gitdir, remote, ref }) {
  ...
  let commit = await managers_js.GitObjectManager.read({ gitdir, oid });
  if (commit.type !== 'commit') {
    throw new Error(`Unexpected type: ${commit.type}`);
  }
  let comm = models_js.GitCommit.from(commit.object.toString('utf8'));
  let sha = comm.headers().tree;
  // Get top-level tree
  let { type, object } = await managers_js.GitObjectManager.read({ gitdir, oid: sha });
...
async function constructTree({ gitdir, inode }) /*: string */{
  ...
  const tree = models_js.GitTree.from(entries);
...
async function getConfig({ gitdir, path: path$$1 }) {
  const config = await managers_js.GitConfigManager.get({ gitdir });
...
async function fetchPackfile({
  ...
  let oids = await managers_js.GitShallowManager.read({ gitdir });
  ...
  response.packetlines.pipe(thru(async (data, next) => {
    ...
    if (line.startsWith('shallow')) {
      ...
      oids.add(oid);
      await managers_js.GitShallowManager.write({ gitdir, oids });
...
async function init(gitdir /*: string */) {
  ...
  await utils_js.mkdirs(folders);
  await utils_js.write(gitdir + '/config', '[core]\n' + '\trepositoryformatversion = 0\n' + '\tfilemode = false\n' + '\tbare = false\n' + '\tlogallrefupdates = true\n' + '\tsymlinks = false\n' + '\tignorecase = true\n');
  await utils_js.write(gitdir + '/HEAD', 'ref: refs/heads/master\n');
...
async function setConfig({ gitdir, path: path$$1, value }) {
  ...
  await managers_js.GitConfigManager.save({ gitdir, config });
}
