...
import * as GitConfigManager from './GitConfigManager'
...
export async function writeRef ({ fs: _fs, gitdir, ref, value }) {
  const fs = new FileSystem(_fs)
  // Validate input
  if (!value.match(/[0-9a-f]{40}/)) {
    throw new Error(`Unexpected ref contents: '${value}'`)
...
export async function resolve ({ fs: _fs, gitdir, ref, depth }) {
  const fs = new FileSystem(_fs)
  if (depth !== undefined) {
    depth--
    if (depth === -1) {
  ...
  let packedMap = await packedRefs({ fs, gitdir })
  ...
  for (let ref of allpaths) {
    sha =
      ...
      packedMap.get(ref)
    if (sha) {
      return resolve({ fs, gitdir, ref: sha.trim(), depth })
    }
  }
  // Do we give up?
...
export async function expand ({ fs: _fs, gitdir, ref }) {
  const fs = new FileSystem(_fs)
  // Is it a complete and valid SHA?
  if (ref.length === 40 && /[0-9a-f]{40}/.test(ref)) {
    return ref
...
export function resolveAgainstMap ({ ref, fullref = ref, depth, map }) {
  if (depth !== undefined) {
    depth--
    if (depth === -1) {
  ...
  if (ref.startsWith('ref: ')) {
    ref = ref.slice('ref: '.length)
    return resolveAgainstMap({ ref, fullref, depth, map })
  }
  // Is it a complete and valid SHA?
  if (ref.length === 40 && /[0-9a-f]{40}/.test(ref)) {
    return { fullref, oid: ref }
...
export async function packedRefs ({ fs: _fs, gitdir }) {
  const refs = new Map()
  const fs = new FileSystem(_fs)
...
export async function listRefs ({ fs: _fs, gitdir, filepath }) {
  const fs = new FileSystem(_fs)
  let packedMap = packedRefs({ fs, gitdir })
  let files = null
  try {
...
export async function listBranches ({ fs: _fs, gitdir, remote }) {
  const fs = new FileSystem(_fs)
  if (remote) {
    return listRefs({
      fs,
    ...
    return listRefs({ fs, gitdir, filepath: `refs/heads` })
  }
}

export async function listTags ({ fs: _fs, gitdir }) {
  const fs = new FileSystem(_fs)
  let tags = await listRefs({
    fs,
    gitdir,
    filepath: `refs/tags`
  })
