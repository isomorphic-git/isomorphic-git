...
export async function read ({ fs: _fs, gitdir, oid, format = 'content' }) {
  ...
  if (!file) {
    ...
    const getExternalRefDelta = oid => read({ fs: _fs, gitdir, oid })
...
export async function write ({ fs: _fs, gitdir, type, object }) {
  const fs = new FileSystem(_fs)
