...
import * as GitRemoteConnection from './GitRemoteConnection'
...
export async function discover ({ service, url, auth }) {
...
export async function connect ({ service, url, auth, stream }) {
  // Auto-append the (necessary) .git if it's missing.
  if (!url.endsWith('.git')) url = url += '.git'
  let headers = {}
