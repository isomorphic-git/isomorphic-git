...
let lock = new AsyncLock({ maxPending: Infinity })
...
/**
 * Discard the shared lock and start a fresh one. Intended for test harnesses
 * that run many independent scenarios inside one long-lived module instance: if
 * a scenario leaves an operation stalled (e.g. a filesystem that hangs), its
 * still-held lock would otherwise block every later scenario that touches the
 * same path. Resetting abandons that stalled queue so the next scenario starts
 * clean. It is a no-op for normal library use.
 */
