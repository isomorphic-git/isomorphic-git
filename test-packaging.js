...
export async function createTempPackage() {
  ...
  await writeFile(join(cwd, 'package.json'), JSON.stringify({ 
    private: true, workspaces: ["../","../packages/types","../packages/isomorphic-git"], name: 'test-pkg', version: "0.0.0-pre.0", type: "module" 
  }, null, 2));
  ...
  tryCatch( () => execSync(`node -e "console.log(require('isomorphic-git'))"`, { cwd }) ),
  tryCatch( () => execSync(`node -e "import('isomorphic-git').then(console.log)"`, { cwd }) ),
  tryCatch( () => execSync(`node -e "console.log(require.resolve('isomorphic-git'))"`, { cwd }) ),
  tryCatch( () => execSync(`node -e "console.log(import.meta.resolve('isomorphic-git'))"`, { cwd }) )
  ]
  return testResults;
}
