...
const { cores, uploadPack, collect } = require('isomorphic-git/internal-apis')

describe('uploadPack', () => {
  it('advertiseRefs: true', async () => {
    // Setup
    const { core, fs, gitdir } = await makeFixture('test-uploadPack')
    cores.create(core).set('fs', fs)
    const res = await uploadPack({ core, gitdir, advertiseRefs: true })
